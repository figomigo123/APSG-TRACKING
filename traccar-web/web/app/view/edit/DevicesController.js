...
Ext.define('Traccar.view.edit.DevicesController', {
    ...
    onSearchTextChange: function () {
        ...
        // console.log(this.lookupReference("searchDevice").value);
        // console.log(this.getView().store.data.items);

        var searchTxt = this.lookupReference("searchDevice").value;
        var deviceList = Ext.getStore('Devices');
        deviceList.clearFilter();
        if (searchTxt) {
            deviceList.filterBy(function (device) {
                var str = (device.get('name')).toLowerCase() + (device.get('uniqueId')).toLowerCase() ;
                
                var res = str.indexOf(searchTxt.toLowerCase());
